-- SQL Fix: Harden draw_winner_and_payout to strictly enforce goal_tibs
...
BEGIN
    ...
    SELECT host_user_id, title, entry_cost_tibs, goal_tibs 
    INTO v_host_id, v_event_title, v_entry_cost, v_goal_tibs
    ...
    IF v_goal_tibs > 0 AND v_total_pot < v_goal_tibs THEN
        RETURN jsonb_build_object(
            'success', false, 
            'message', 'GOAL NOT REACHED: Current pot is ' || v_total_pot || ' but goal is ' || v_goal_tibs || '. You must refund instead.'
        );
    END IF;

    -- 4. Get pick winner entry
